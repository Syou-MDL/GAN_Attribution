...
class Projector:
    def __init__(self, loss_func='lpips',crop_size=None):
        self.num_steps                  = 1000
...
#         assert minibatch_size == 1
        ...
        dlatent_samples = np.random.RandomState(123).randn(self.dlatent_avg_samples, *self._Gs.input_shapes[0][1:])
#         dlatent_samples = self._Gs.components.mapping.run(latent_samples, None)[:, :1, :] # [N, 1, 512]
        ...
        self._dlatents_expr = tf.tile(self._dlatents_var + dlatents_noise, [1, 1])
        self._labels = np.zeros([self._minibatch_size, 0], np.float32)
        self._images_expr = self._Gs.get_output_for(self._dlatents_expr,self._labels , randomize_noise=False)
        if self.crop_size is not None:
            self.crop_size=int(self.crop_size)
            self._images_expr=tf.transpose(self._images_expr, perm=[0,2, 3, 1])
            self._images_expr=tf.image.resize_image_with_crop_or_pad(self._images_expr, self.crop_size, self.crop_size)
            self._images_expr=tf.image.resize_image_with_crop_or_pad(self._images_expr, 128, 128)
            self._images_expr=tf.transpose(self._images_expr, perm=[0,3, 1, 2])
            print( self._images_expr)
        ...
        # Loss graph.
        self._info(f'Building loss graph using {self.loss_func}...')
...
#             self._lpips = misc.load_pkl('https://nvlabs-fi-cdn.nvidia.com/stylegan/networks/metrics/celebahq-classifier-00-male.pkl')
        
        self._dist = self._lpips.get_output_for(proc_images_expr, self._target_images_var)
        self._l2 = tf.reduce_mean(tf.math.square(proc_images_expr - self._target_images_var),axis=[1,2,3])
        if self.loss_func=='l2':
            self._loss = tf.reduce_mean(self._l2)
        elif self.loss_func=='lpips':
    ...
    def step(self):
        ...
        if self._cur_step == self.num_steps or self._cur_step % 10 == 0:
            self._info('%-8d%-12g%-12g%-12g' % (self._cur_step, learning_rate, np.mean(dist_value), loss_value))
        if self._cur_step == self.num_steps:
            self._info('Done.')
    ...
    def get_l2(self):
        return tflib.run(self._l2, {self._noise_in: 0})
