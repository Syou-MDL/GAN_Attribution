...
def train_progressive_gan(
    ...
    image_snapshot_ticks    = 100,          # How often to export image snapshots?
...
def re_train_progressive_gan(
    ...
    save_tf_graph           = False,        # Include full TensorFlow computation graph in the tfevents file?
    save_weight_histograms  = False,      	# Include weight histograms in the tfevents file?
    model_pkl_path = 'models/celeba_align_png_cropped_seed_v0.pkl',
    ...
    # Construct networks.
    with tf.device('/gpu:0'):
        print('retrain network. model_path:%s',model_pkl_path)
        network_pkl = misc.locate_network_pkl(model_pkl_path)
        resume_kimg = 12000
        resume_time = 0
        print('Loading networks from "%s"...' % network_pkl)
        G, D, Gs = misc.load_pkl(network_pkl)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total_kimg', type=int, default=12000) # The output directory containing trained models, training configureation, training log, and training snapshots    
    ...
    parser.add_argument('--run_id', type=int, default=8)
    ...
    if args.app == 'train':
        ...
        app = config.EasyDict(func='run.train_progressive_gan', mirror_augment=False, total_kimg=args.total_kimg)
        config.result_dir = args.out_model_dir
    elif args.app == 'retrain':
        assert args.training_data_dir != ' ' and args.out_model_dir != ' '
        misc.init_output_logging()
        np.random.seed(args.training_seed)
        print('Initializing TensorFlow...')
        ...
        config.data_dir = args.training_data_dir[:idx]
        config.dataset = config.EasyDict(tfrecord_dir=args.training_data_dir[idx+1:])
        app = config.EasyDict(func='run.re_train_progressive_gan', mirror_augment=False, total_kimg=args.total_kimg)
        config.result_dir = args.out_model_dir
    elif args.app == 'gen':
    ...
    elif args.app == 'gen_all':
        assert args.run_id != ' ' and args.out_image_dir != ' '
        misc.init_output_logging()
        np.random.seed(args.gen_seed)
        print('Initializing TensorFlow...')
        os.environ.update(config.env)
        tfutil.init_tf(config.tf_config)
        app = config.EasyDict(func='util_scripts.generate_fake_images_all', run_id=args.run_id, out_dir=args.out_image_dir, num_pngs=args.num_pngs, random_seed=args.gen_seed)
    elif args.app == 'recover_latent':
        misc.init_output_logging()
        np.random.seed(args.gen_seed)
        print('Initializing TensorFlow...')
        os.environ.update(config.env)
        tfutil.init_tf(config.tf_config)
        app = config.EasyDict(func='util_scripts.recover_latent', pkl_path=args.model_path, out_dir=args.out_image_dir, num_pngs=args.num_pngs, random_seed=args.gen_seed)
