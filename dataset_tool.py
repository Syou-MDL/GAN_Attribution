...
#         X = data['data'].reshape(-1, 3, 32, 32).transpose(0,2,3,1).astype("uint8")
...
#     print(images.shape)
#     print(images.dtype)
#     assert images.shape == (50000, 3, 128, 128) and images.dtype == np.uint8
#     assert labels.shape == (50000,) and labels.dtype == np.int64
#     assert np.min(images) == 0 and np.max(images) == 255
#     assert np.min(labels) == 0 and np.max(labels) == 9
#     onehot = np.zeros((labels.size, np.max(labels) + 1), dtype=np.float32)
...
# +
def create_celeba(tfrecord_dir, celeba_dir, cx=89, cy=121, shuffle=1, num_images=10000, num_shifts=0):
    print('Loading CelebA from "%s"' % celeba_dir)
    print('num_img='+str(num_images))
...
def create_celeba_noise(tfrecord_dir, celeba_dir, cx=89, cy=121, shuffle=1, num_images=1000, num_shifts=0):
    print('creat celeba 1k noise')
    print('Loading CelebA from "%s"' % celeba_dir)
    glob_pattern = os.path.join(celeba_dir, '*.jpg')
    image_filenames = sorted(glob.glob(glob_pattern))
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames_new)) as tfr:
        ...
        for idx in range(order.size):
            ...
            assert img.shape == (218, 178, 3)
            img = img[cy - 64 : cy + 64, cx - 64 : cx + 64]
            noised_image = addGaussianNoise(img)
            noised_image = noised_image.transpose(2, 0, 1) # HWC => CHW
            tfr.add_image(noised_image)
            
def create_celeba_rotate(tfrecord_dir, celeba_dir, cx=89, cy=121, shuffle=1, num_images=1000, num_shifts=0,angle=15):
    print('creat celeba 1k noise')
    print('Loading CelebA from "%s"' % celeba_dir)
    glob_pattern = os.path.join(celeba_dir, '*.jpg')
    image_filenames = sorted(glob.glob(glob_pattern))
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames_new)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames_new))
        for idx in range(order.size):
            img = np.asarray(PIL.Image.open(image_filenames_new[order[idx]]).rotate(angle))
            assert img.shape == (218, 178, 3)
            img = img[cy - 64 : cy + 64, cx - 64 : cx + 64]
            img = img.transpose(2, 0, 1) # HWC => CHW
            tfr.add_image(img)
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_celeba_noise',    'Create dataset for CelebA.',
                                            'create_celeba datasets/celeba ~/downloads/celeba')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'celeba_dir',       help='Directory containing CelebA')
    p.add_argument(     '--cx',             help='Center X coordinate (default: 89)', type=int, default=89)
    p.add_argument(     '--cy',             help='Center Y coordinate (default: 121)', type=int, default=121)
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
    p.add_argument(     '--num_images',     help='Number of images to be considered (default: all)', type=int, default=1000)
    p.add_argument(     '--num_shifts',     help='Number of image index shifts to be considered (default: 0)', type=int, default=0)
    
    p = add_command(    'create_celeba_rotate',    'Create dataset for CelebA.',
                                            'create_celeba datasets/celeba ~/downloads/celeba')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'celeba_dir',       help='Directory containing CelebA')
    p.add_argument(     '--cx',             help='Center X coordinate (default: 89)', type=int, default=89)
